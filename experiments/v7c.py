...
params = {
    ...
    'verifythresh': 50,
    'freqthresh': 5,
    'base_ckpt_path': ROOT + 'experiments/v7/ep4_batch_size-32_epochs-5_pooling-G,G,G,G.pth',
}
...
def job(tuning, params_path, devices, resume, save_interval):
    ...
    if resume is not None:
        params['clean_path'] = utils.load_checkpoint(path=resume, params=True)['params']['clean_path']
    ...
    sdict = torch.load(params['base_ckpt_path'])['state_dict']
    ...
    if resume is not None:
        ckpt = utils.load_checkpoint(path=resume, model=model, optimizer=optimizer, epoch=True, params=True)
        model, optimizer, start_epoch, params = ckpt['model'], ckpt['optimizer'], ckpt['epoch'] + 1, ckpt['params']
    ...
    results = eval_datasets(model, datasets=datasets, ms=False, tta_gem_p=1.0, logger=logger)
