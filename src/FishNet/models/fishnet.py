...
class Fish(nn.Module):
    ...
    def _fish_forward(self, all_feat, calc_score=True):
        def _concat(a, b):
        ...
        while stg_id < self.depth:
            ...
            if stg_id == self.depth:
                ...
                if calc_score:
                    ...
                    return score
                # else:
                #     all_feat[-1] = score_feat
        return all_feat

    def forward(self, x, calc_score=True):
        all_feat = [None] * (self.depth + 1)
        all_feat[0] = x
        return self._fish_forward(all_feat, calc_score=calc_score)
...
class FishNet(nn.Module):
    ...
    def features(self, x):
        """extract feature with same api as pretrainedmodels"""
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
        x = self.pool1(x)
        all_feat = self.fish(x, calc_score=False)
        return all_feat[7:]
